...
class RequestBasedFileHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        url_path = getattr(record, 'url_path', 'unknown').replace('/', '_').strip('_') or 'root'
...
class RequestLoggingMiddleware:
    ...
    def __call__(self, request):
        ...
        for logger_name in ['django', 'django.request', 'django.server', 'guest_user']:
            ...
            logger.filters = [f for f in logger.filters if not isinstance(f, RequestFilter)]
            logger.addFilter(RequestFilter())
        
        # Log request start
        logger = logging.getLogger('django.request')
        logger.info(f"=== REQUEST START === {request.method} {request.path} [User: {user_type}] [ID: {request_id}]")
        
        response = self.get_response(request)
        
        # Log request end
        logger.info(f"=== REQUEST END === {request.method} {request.path} [Status: {response.status_code}] [ID: {request_id}]")
