...
def test_delete_expired_users_with_expired():
    ...
    # Create some expired guest users and get their Guest instances
    expired_guest_instances = []
    for _ in range(3):
        user = GuestModel.objects.create_guest_user()  # Returns User object
        # Get the actual Guest instance
        guest_instance = GuestModel.objects.get(user=user)
        expired_guest_instances.append(guest_instance)

    # Force update the created_at timestamp to make them expired
    old_timestamp = now() - timedelta(days=25)  # Default MAX_AGE is ~2 weeks
    GuestModel.objects.filter(id__in=[g.id for g in expired_guest_instances]).update(
...
def test_delete_expired_users_custom_max_age():
    ...
    GuestModel.objects.filter(id=guest_instance.id).update(created_at=old_timestamp)
